...
test('Cavecrew investigator and reviewer use the requested GPT Sol variants', () => {
  const investigator = readFileSync(
    new URL('../config/agents/cavecrew-investigator.md', import.meta.url),
    'utf8',
  );
  const reviewer = readFileSync(
    new URL('../config/agents/cavecrew-reviewer.md', import.meta.url),
    'utf8',
  );
  assert.match(
    investigator,
    /^model: 9router-sol\/cx\/gpt-5\.6-sol\r?\nvariant: low$/m,
  );
  assert.match(
    reviewer,
    /^model: 9router-sol\/cx\/gpt-5\.6-sol\r?\nvariant: medium$/m,
